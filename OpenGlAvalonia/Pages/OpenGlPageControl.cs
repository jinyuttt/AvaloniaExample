...
namespace OpenGlAvalonia.Pages
{
...
");
        ...
        protected override unsafe void OnOpenGlInit(GlInterface GL)
        {
            ...
            fixed (void* pdata = _points)
                GL.BufferData(GlConsts.GL_ARRAY_BUFFER, new nint(_points.Length * vertexSize),
                    new nint(pdata), GlConsts.GL_STATIC_DRAW);

            _indexBufferObject = GL.GenBuffer();
            GL.BindBuffer(GlConsts.GL_ELEMENT_ARRAY_BUFFER, _indexBufferObject);
            CheckError(GL);
            fixed (void* pdata = _indices)
                GL.BufferData(GlConsts.GL_ELEMENT_ARRAY_BUFFER, new nint(_indices.Length * sizeof(ushort)), new nint(pdata),
                    GlConsts.GL_STATIC_DRAW);
            CheckError(GL);
            _vertexArrayObject = GL.GenVertexArray();
            GL.BindVertexArray(_vertexArrayObject);
            CheckError(GL);
            GL.VertexAttribPointer(positionLocation, 3, GlConsts.GL_FLOAT,
                0, vertexSize, nint.Zero);
            GL.VertexAttribPointer(normalLocation, 3, GlConsts.GL_FLOAT,
                0, vertexSize, new nint(12));
        ...
        protected override unsafe void OnOpenGlRender(GlInterface gl, int fb)
        {
            ...
            CheckError(GL);
            GL.DrawElements(GlConsts.GL_TRIANGLES, _indices.Length, GlConsts.GL_UNSIGNED_SHORT, nint.Zero);
